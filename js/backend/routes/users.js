...
usersRouter.get(`${sql_api_path}/verify`, async (req, res) => {
  try {
...
usersRouter.post(`${sql_api_path}/login`,  async(req, res) => {
  ...
  try {
    ...
    if (db_res.valid === true){
      ...
      console.log(user)
      const user_token = jwt.sign({ id: user.id, isadmin: user.isadmin }, SEC_KEY, { expiresIn: '90d' })
